    ...
    <div class="row my-5">      
        <div class="card col-md-9 m-auto shadow-sm p-4 my-5">         
            <div class="card-header text-white bg-info mx-5 mt-4 text-center py-3 h4">
                <span class="float-start ms-2"><a href="{{ route('admin.sent_messages_table') }}" class="link-secondary"><i class="bi bi-x-square"></i></a></span>
                Sent Message
                @if($message->receiver_status === 0) 
                    <span class="float-end me-2">
                        <form method="POST" action="{{ route('admin.delete_sent_message') }}" style="display:inline-block">
                            @csrf
            ...
            <div class="card-body text-dark bg-white mx-5 px-4 fs-5 border">
                <div class="card-text fw-bold"> Status : </div>
            ...
            <div class="card-body text-dark bg-white mx-5 px-4 fs-5 border">
                <div class="card-text fw-bold"> Sent To : </div>
                <div class="card-text ms-4"> {{$message->receiver->fname}} {{$message->receiver->lname}}</div>                 
            </div>
            <div class="card-body text-dark bg-white mx-5 px-4 fs-5 border">
                <div class="card-text fw-bold"> Sent By : </div>
                <div class="card-text ms-4"> {{$message->adminSender->name}} </div>                 
            </div>
